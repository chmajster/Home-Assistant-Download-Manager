...
Na stronie **Zadania** zwykłe pobieranie można zatrzymać i wznowić. Zatrzymanie zachowuje pliki częściowe `yt-dlp`, a wznowienie uruchamia ten sam URL i wariant formatu z aktywną obsługą kontynuacji pobierania. Filtr **Błędy** pokazuje tylko nieudane zadania, a panel błędów podpowiada najczęstsze przyczyny. Przy pojedynczym błędnym zadaniu można kliknąć **Ponów**, a przycisk **Ponów nieudane** uruchamia ponownie wszystkie zadania ze statusem `błąd`.
